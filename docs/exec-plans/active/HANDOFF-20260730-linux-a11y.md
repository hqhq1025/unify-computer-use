...
**通道有两个正交的轴，必须分开报**：执行轴（语义动作 vs 合成）与观测轴
（a11y 树 vs 截图）。以前只有执行轴一个数，说不出"这次任务有多依赖截图"。

```
成功率 5/5 = 100%   平均步数 3.2   平均 token 15352（文本 10232 + 视觉 5120）
执行轴 a11y 52%（9/17 条动作 Note）   观测轴 视觉 33% token（27/27 次观测带图）
```

| 任务 | 工具包 | 步数 | 文本 token | 视觉 token | 执行 a11y |
|---|---|---|---|---|---|
| thunderbird-folder | Gecko | 1 | 6640 | 5391 | 100% |
| vscode-edit | Electron | 4 | 22679 | 6950 | **0%** |
| nautilus-rename | GTK | 5 | 8287 | 5548 | 66% |
| vlc-preference | Qt | 3 | 12282 | 3656 | 100% |
| gedit-type | GTK | 3 | 1271 | 4056 | 33% |

> 加入 Electron 前是 69% / 7026 token。**把最不可靠的工具包排除在外，
> 指标会显著好看于实际**——这个基线是 A/B 的基准，抽样偏了结论就全错。

> **截图默认恒带的代价已实测**：总 token 10232 → 15352（**+50%**），
> 成功率与步数不变。gedit 上截图是树的 **3.2 倍**（文本 1271 / 视觉 4056），
> 小应用上截图就是主要成本——这与"a11y 树未必更便宜"是同一件事的两面。
> 关掉用 `OPEN_COMPUTER_USE_A11Y_SCREENSHOTS=0`，`drag` 不受影响（恒带）。
