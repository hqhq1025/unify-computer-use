...
### 从 OSWorld 跑测里长出来的（已完成，按发现顺序）

跑到第 30 题时的账。**这些缺陷我自己撞不出来**——我卡住会停下来查根因，
agent 卡住会换一条路，于是它遇到的阻力不变成报错，只变成多余的步数。
它们全部来自 `scripts/analyze-agent-traces.py` 对真实轨迹的统计。

| 发现 | 证据 | 修法 |
|---|---|---|
| 应用名有三种风格且无规则可推 | 13 条轨迹里 **12 条**开局是 `list_apps → get_app_state` | 分隔符不敏感匹配 + 失败时列候选 + 命名规则写进 12 个工具的 `app` 说明 |
| 光改描述没用 | 对照跑：仍然例行先列 | **`app` 改成可选**，留空即当前前台。修复后 `list_apps` 用量 15 → **0** |
| 树预算的单位错了 | 60 次观测里 **8 次（13%）**响应 114–119KB，被客户端换成读不了的文件指针 | 加**以字符计**的预算，遍历时就停手 |
| `showContextMenu` 让剪枝失效 | 它出现在**全部 1200 个节点**上 | 归入 `UNIVERSAL_ACTIONS`，不进动作列表、不阻止剪枝 |
| 41% 的文本叶子重复父节点 | 521 个纯文本节点里 212 个逐字相同 | 无子节点且文字与父相同的不渲染 |
| 遍历没有时间上限 | GIMP 8.45ms/节点 × 3162 = 38s > 30s 超时 | 借鉴 maka-cu 加墙钟 deadline（根节点豁免） |
| `verify` 失败说不清 | 5 次调用失败 3 次，**三次全是 `exists:true` 超时** | 失败时列出最接近的候选 |
| `verify(exists=false)` 是坏代理 | 第 28 题连按四次、15→20→25→20 秒，白烧 80 秒 | 界面没动时指出这一点并建议"等新东西出现" |
| `set_value` 只会硬失败 | Blink 的输入框不实现 AT-SPI Value 接口 | 补上聚焦→全选→打字的降级，并声明无回读确认 |
| 焦点守卫的**假阳性** | 门户对话框开着，输入落进了发起它的 Chrome | 抢焦点失败后请求 WM 激活，**并重新确认**才返回成功 |
| `main_window` 选错窗口 | 门户两个窗口状态位完全相同 | 让 X11 按尺寸裁决（减掉 `_GTK_FRAME_EXTENTS`）|

**三条查完是虚惊、没有乱改的**（记下来同样重要，免得下次再查一遍）：

- `find` 零命中率 21%（6/29）——查了三个假设，**全部证伪**：
  印刷体撇号？树里就是 ASCII `0x27`。
  部分名字匹配坏了？`role=button` + `name=minim` 实测命中 `push button "Minimize"`。
  角色名带空格？`document web` 实测 find/verify 都正常。
  结论：零命中大多是 agent 在**探索**它不确定存不存在的东西，不是缺陷。

- `press_key` 重复 14 次 —— 逐条看是回车提交、重新导航，正常流程
- `click → click_xy` 88% —— **是我的指标算错了**。它统计"整条轨迹里同时出现过
  两者"；逐条看直接前一步，`click_xy` 大多正当（连续坐标 8、零效果兜底 3、
  截图后按图点 2），真正"绕开语义"只有 1 次。指标已修。
  **把正常行为算成病症的指标，比没有指标更糟。**
